...
$Col2 =  New-Object system.Data.DataColumn("Users",([string]))
...
#Get system data
$MBUsers = Get-Mailbox -resultsize Unlimited | Group-Object -Property:Database | Select-Object name,count

#Build datatable
foreach ($MB in $MBUsers) {
	$row = $output.NewRow();	
		$row.Database = $MB.Name;
		$row.Users = $MB.Count;
		$row.Year = $Year;
		$row.Month = $Month;
		$row.Day = $Day;
