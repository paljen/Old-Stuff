Function Get-EccoServerDriveInformation
{
    ...
    .EXAMPLE 
        (Get-ADComputer -Filter * -SearchBase "OU=Application,OU=Member servers,dc=prd,dc=eccocorp,dc=net").Name | 
            Get-EccoServerDriveInformation -Filter C:
...
(Get-ADComputer -Filter * -SearchBase "OU=Application,OU=Member servers,dc=prd,dc=eccocorp,dc=net").Name | test -Filter C: