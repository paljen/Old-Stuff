...
$ADusers = Get-ADUser -SearchBase "OU=ECCO,DC=PRD,DC=ECCOCORP,DC=NET" -Filter * -Properties msDS-cloudExtensionAttribute1 | Where-Object {($_.enabled -eq $false) -and ($_."msDS-cloudExtensionAttribute1" -eq "E3")}
#$ADusers = Get-ADUser -SearchBase "OU=ECCO,DC=PRD,DC=ECCOCORP,DC=NET" -Filter * -Properties msDS-cloudExtensionAttribute1 | Where-Object {($_.enabled -eq $True) -and ($_.UserPrincipalName -like "*ecco.com") -and ($_.DistinguishedName -notlike "*Terminated*") -and ($_.DistinguishedName -notlike "*PreStaging*") -and ($_.DistinguishedName -notlike "*Production*")}
#$count = 0
...
#$date = Get-Date
#$outstring = "Completed: $date" | Out-File C:\Toolbox\O365\Attrib.txt