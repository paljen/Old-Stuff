<#
.SYNOPSIS
  Storage requrement for USMT migration

.DESCRIPTION
  Return and store USMT Scanstate information
...
.OUTPUTS Log File
  The script log file stored in $env:temp\USMTClientMigration.log
...
# Delete old log file
"$LogPath\USMTClientMigration.log" | Remove-Item -ErrorAction SilentlyContinue
...
function Get-ComputerData
{
    ...
    # Retrive USTM Size from XML
    [XML]$Size = Get-Content "$logpath\Size.xml"
    ...
    ## Create hashtable with scorch runbook properties
    return $props = @{
               'ComputerName'=$cn;
               'PrimaryUser'=$pu;
               'USMTSize'= ($($size.PreMigration.storeSize.size.GetValue(0).'#text') / 1MB) -as [Int];
    }
}
...
Function Main
{
    try
    {
        ...
        ## Import Scorch module
        Import-Module $("$ScriptPath\scorch\scorch.psd1")
		
        ## Declare Scorch Runbook information
	    $rbGuid = "4b957957-bb87-4710-84bc-1ad591ecb793"
	    $rbServer = "DKHQSCORCH01.PRD.ECCOCORP.NET" #"10.129.12.64"
        
        Write-LogFile "Create SCOWebserverURL for server: $($rbServer)"
		$rbWebURL = New-SCOWebserverURL -ServerName $rbServer
        
        Write-LogFile "Creating Computer Data Object"

        ## Garther Computer Information
        $rbparams = Get-ComputerData

        Write-LogFile "Starting Runbook with ID: $($rbGuid) and Parameters: $($rbParams.get_Item('Computername')) $($rbParams.get_Item('PrimaryUser')) $($rbParams.get_Item('USMTSize'))"
    ...
    catch
    {
	    ...
	    Write-LogFile "Exception caught: $ErrorMessage"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUIs9ok62jgHuWQNxgHLHtDwtH
# ikGgggyvMIIGEDCCBPigAwIBAgITMAAAACpnbAZ3NwLCSQAAAAAAKjANBgkqhkiG
...
# NwIBFTAjBgkqhkiG9w0BCQQxFgQUu+EfJr2fZHWWUnyk8HCb07T23GowDQYJKoZI
# hvcNAQEBBQAEggEAsiPr++agTwvUj7U4cHBqYWIOFBDOSyqkrIHW7VEAb7fdA77c
# TxEYZAut+PbT7Q0mLqOCHopinTntD/i6xyjNm4ShuNwsGLAQfIjWDdoH+/d6AOwG
# +72rrNq/Jy8lYQiaOoLKkdZJ7HWhFnyMTLC/zPpkPVpqBhTnAJ3TP68rykfiNrid
# vf3K/FoFao6HderiMQs0FvJE5gywY1Kn37TCttKazBoyEsNAy0Y/a6NvDfG/ZuIW
# ZvGUY3huH+I3y691bYC/j7XlKMGXFTdDE39fLlYXP5WY9R8jgeVv5zjZVgfcXk+U
# +tzfvUOqic1AR+B/+5d5wUlIaKhx+KX732Zjmw==
