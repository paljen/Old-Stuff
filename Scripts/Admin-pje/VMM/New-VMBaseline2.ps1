    ...
    try 
    {
      ...
      #------------------------------------------------------------
        ...
        Invoke-Command -Session $session -ArgumentList $args -ScriptBlock { 
            param($vhdpath,$vhdsize,$vmname,$mac,$memory)
        ...
        Write-LogFile "Get-VMNetworkAdapter -VMName $vmname | Set-VMNetworkAdapter -StaticMacAddress $mac" -Trace
        ...
        Write-LogFile "Static Mac Address: [$mac]"         
