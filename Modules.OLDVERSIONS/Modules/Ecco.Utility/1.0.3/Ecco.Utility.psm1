...
.NOTES
	...
	Purpose/Change: Initial script module development
    
    Version:        1.0.1
	Author:         Admin-PJE
	Creation Date:  26/04/16
	Purpose/Change: Added New-EccoPSScript to Templates 
                    Added New-EccoPSWorkflow to Templates
                     
    Version:        1.0.2
	Author:         Admin-PJE
	Creation Date:  28/04/16
	Purpose/Change: Added New-EccoPSCredential to Tools
                    Added New-EccoPSCredentialSnippet to Tools 

    Version:        1.0.3
	Author:         Admin-PJE
	Creation Date:  12/05/16
	Purpose/Change: Added New-EccoPSScriptModule to Templates
#>

Function Get-EccoUtilModuleDirectory {
    $Invocation = Get-Variable MyInvocation -Scope 1 -ValueOnly
    Split-Path -Parent $($Invocation.MyCommand.source)
}

Get-ChildItem (Get-EccoUtilModuleDirectory) -Recurse | Where-Object { $_.Name -like "Func_*" } | %{. $_.FullName} -Verbose
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUDaAZUp1JHt8DqQouFJ04sFtS
# +d2gggyvMIIGEDCCBPigAwIBAgITMAAAACpnbAZ3NwLCSQAAAAAAKjANBgkqhkiG
...
# NwIBFTAjBgkqhkiG9w0BCQQxFgQUThFBuqO6vyZUn4JJxr0N0M6jriswDQYJKoZI
# hvcNAQEBBQAEggEAKX3GkmlAJFFM8qR47uK7j434lfjh9E1Nq0OxjBq6OZLyOXmo
# E9y1gQhmohTZ5OgVkvSweWrFsg7hPdmNfG6oVjFgSxDfjYuSmLZCKdoesHT6fLrE
# BSvwC4N/QdSBsdISfFqXJpUhbWwbjxsiRVXzsROA05HooWdv0fY6J1+YLajY77DL
# cqcK5z0N/+JSCcGkwYCqK2/bIm6yIGY4dM7toG4A0oD47xM4+EXXSO58ie8iYtQU
# tKmKlbGP0rLVjthPHZBDna+xfp2pHTwo1+0c5dMC4y/bptoaA4ZQ8QMxFiLuytyY
# xf0tYVlEbYLQK+aQ1qPMcFgjuRHCDCVaAeA87A==
