...
Function Out-EcGlobalCSV
{
    <#
    .SYNOPSIS
	    Splits output into several csv files.

    .DESCRIPTION
	    Splits output into several csv files.

    .EXAMPLE
        $adusers = Get-ADUser -SearchBase $sBase  -Filter {Enabled -eq $True} -properties EmployeeId | ? EmployeeId -ne $null | select -ExpandProperty UserPrincipalName
        $mb = $adusers | % {Get-Mailbox $_ -ResultSize Unlimited -ErrorAction Ignore | select UserPrincipalName}
        Split-CSV -source $mb -Split 500 -path $path
    ...
    .PARAMETER  Split
	    The number of elements in each file

    .PARAMETER  Path
	    The path and filename on the files
    ...
    $tmp = "c:\temp\temp.csv"
    $source | Export-Csv $tmp
    $csv = Import-Csv $tmp
      
    $stop = [System.Math]::Ceiling($source.Count/$split)
    $count = $source.Count
    ...
    for ($i = 0; $i -lt $stop ; $i++)
    {
        ...
        {
            $Count -= $split
            $csv | Select -Last $split | Export-Csv "$path-$i.csv" -NoTypeInformation
            $csv | Select -First $Count | Export-Csv $tmp -NoTypeInformation -Force
            $csv = Import-Csv $tmp
        }
        else
        {
            $csv | Export-Csv "$path-$i.csv" -NoTypeInformation
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU7Fw4KmHkOy3c5wh3tzgn06Um
# 9wOgggyvMIIGEDCCBPigAwIBAgITMAAAACpnbAZ3NwLCSQAAAAAAKjANBgkqhkiG
...
# NwIBFTAjBgkqhkiG9w0BCQQxFgQUc3OqNj67ZT0W4N7r3wX3ph4BpsAwDQYJKoZI
# hvcNAQEBBQAEggEAL1zCBX9XUmXkaySwvb/l3rL8rGse7ll9rA8kHSBgu92lIVH7
# YZEjiuNnNY4MUSaVvbMdB5sW+f52xerDPNvVrsFb4Po9tOgBcouHAe/WlwxCgTt1
# cGt03s+juKFzEXJV+dEIAEnlvNQEPBoyNb651a8mzYNDNL448ZphB7sBD2NOGTG7
# Pwyqj6yuKVw9kx4CSP6qwe420wxOM/jYZnkkoGIkqHSPEObuYDrhPQFTeo1tpWxh
# k7GJYFsrcNi98C9N6pteObkneQnbyTah9Lb+DsM1pI8j0GlFg5Z6zWr2IxPjFw5c
# y6+GI5NopptyKZ4MX7qqGg1GGNYK582pygLLiQ==
