param($webServer, $port, $runbookPath, $paramString, $waitUntilDone, $debug)

$SCRIPT_NAME = 'StartRunbook.ps1'
...
$message =  "Script started." + "`n" + `
		...
		"Port: " + $port + "`n" + `
		"Runbook: " + $runbookPath + "`n" + `
		"Parameters: " + $paramString + "`n" + `
		"Wait until done: " + $waitUntilDone
Log-DebugEvent -eventNo 401 -message $message

if ($paramString.length -eq 0) { $paramString = $null}
if ($paramString -ne $null) 
{
write-host "not null"
    # Turn the parameter string into a hashtable
    $rbParameters = @{}
    $paramArray = $paramString.Split(';')
    foreach ($p in $paramArray)
    {
        $rbParameters.Add($p.Split('=')[0],$p.Split('=')[1])
    }
}    

# Find the runbook so we can get it's ID
$baseUrl = -join ((Get-OrchestratorServiceUrl -Server $webServer -Port $port),"/Runbooks")
$queryOptions = -join("?`$filter=Path eq '",$runbookPath,"'")
$url = -join($baseUrl,$queryOptions)
[xml] $xmlDoc = sendHttpGetRequest -Url $Url 
$rbnode = getEntryNodes $xmlDoc
$runbook = getRunbookObject $rbnode   
 
$newJob = Start-OrchestratorRunbook -Runbook $runbook -Parameters $rbParameters
if ($newJob -ne $null)
{
    Write-Host "Created job with id: " $newJob.Id " at " $newJob.CreationTime
}
    
if ($waitUntilDone -eq $true)
{
    do {
        start-sleep -seconds 5
        $job = Get-OrchestratorJob -ServiceUrl $newJob.Url_Service -JobId $newJob.Id
    } until ($job.Status -eq 'Completed' -or $job.Status -eq 'Canceled')
        
    Write-Host "Job finished with status:" $job.Status "- Time finished:" $job.LastModifiedTime
}
