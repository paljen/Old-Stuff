Function New-EcGlobalScriptModuleTemplate
{
    ...
    .EXAMPLE
        New-EcGlobalScriptModuleTemplate -Name SCO
    ...
    .NOTES
        ...
        Purpose/Change: 22/11/2016, UNC Path for ScriptToProcess is to long the path has to be typed manually
    #>
    ...
    $path = $Global:ModuleRepository
    $prefix = "Ecco"
    $author = $env:USERNAME
    $version = "1.0.0"
    $moduleName = "$prefix.$Name.psm1"
    $manifestName = "$prefix.$Name.psd1"
  
    try
    {
        if(Test-path $path\$prefix.$name)
        {
            Throw "Path $path`\$prefix.$name already exist" 
        }

        else
        {
            $parent = New-Item -ItemType Directory -Path $path -Name "$prefix.$Name"
            $child = New-Item -ItemType Directory -Path $parent -Name $version
            New-Item -ItemType Directory -Path $child -Name Scripts
            New-Item -ItemType Directory -Path $child -Name Binary
            $mScript = New-Item -ItemType File -Path $child -Name $moduleName -Force
        }
        ...
        Add-Content $mScript "`tVersion:`t`t$version"
        Add-Content $mScript "`tAuthor:`t`t`t$author"
        Add-Content $mScript "`tCreation Date:`t$(Get-Date -Format dd/MM/yy)"
        Add-Content $mScript "`tPurpose/Change:`tInitial script module development - $moduleName"
        Add-Content $mScript "#>`n"
        Add-Content $mScript "`$ErrorActionPreference = `"Continue`"`n"
        Add-Content $mScript "Get-ChildItem (`"`$Global:ModuleRepository\Ecco.$name\1.0.0\Scripts`") -Recurse -Verbose:`$false | Where-Object -Verbose:`$false { `$_.Name -like `"func.*`" } | Foreach-Object {"
        ...
        $scriptToProcess = "$($path)\GlobalSettings\Variables.ps1"
        New-ModuleManifest -Path "$child\$manifestName" -RootModule $moduleName -Author $author -ModuleVersion $version -CompanyName "ECCO Shoes A/S" -Copyright "(c) 2015 Ecco Shoes A/S. All rights reserved." -ScriptsToProcess $scriptToProcess -PassThru
        New-EcGlobalScriptSigning -GlobalPath $mScript | Out-Null
        New-EcGlobalScriptSigning -GlobalPath "$child\$manifestName" | Out-Null
...
Function New-EcGlobalSCOScriptTemplate
{
}

Function New-EcGlobalSCOInvokeScriptTemplate
{
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU5uQ7jqK9NconlFJyfizw5ULI
# EjegggyvMIIGEDCCBPigAwIBAgITMAAAACpnbAZ3NwLCSQAAAAAAKjANBgkqhkiG
...
# NwIBFTAjBgkqhkiG9w0BCQQxFgQUPb4wbCvGg0L/9yscMyU72tf/w+UwDQYJKoZI
# hvcNAQEBBQAEggEAIv2+3nQ3usdHD+nYwfVbeY6Vs6hZMqgUmJC+Y0rir5Fd/edk
# 7tiBOFI0m5nFgTMiwiNqnOvnJe8J/RN6q2zMIRzVBJgjzYd8JH2ZXbbZ87NoHCra
# jCoM/dqnJhrrDRbFTJqQK/eswsedMI/xJuAaXIfXcydgRTTjMyTsbNVVAUhoa1WH
# JD2QMhj//Jj1kh5Zn71qe7Cp8tp7rBrERoIlrTQfKW0N4Ba2MWAJPxzObusZerLm
# r7vddgJ/jMsYLGsmkA71Q+t3dbEub9d9TFLh1N9mBNBwU7r5ak6C032/5pCTMxzS
# XPoXvig9i9BTjxzGjdMWsDDNOkkr85FseW729w==
