    ...
    try {
		...
		try
		{
			foreach ($dl in $allDL) {
                ...
                (Get-QADGroupMember -Identity $dl -ErrorAction Stop | Where {($_.Classname -eq 'User' -OR $_.Classname -eq 'Contact')}) | ForEach-Object {AppendLog "Removing $($_.SamAccountName) From $dl";Remove-QADGroupMember -Identity $dl -Member $_.DN -Confirm:$false -ErrorAction Stop}
				...
				foreach ($grp in $iGroups) {
                    ...
                    Get-QADGroupMember $grp -ErrorAction Stop | ForEach-Object {AppendLog "Adding $($_.SamAccountName) to $dl";Add-QADGroupMember -Identity $dl $_.DN -ErrorAction Stop}
